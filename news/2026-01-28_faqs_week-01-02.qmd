---
title: "Weeks 1–2 tips and FAQs"
date: "2026-01-28T01:35"
categories: [FAQs]
---
...
Put simply, [the tidyverse](https://tidyverse.org/) is a bunch of R packages that work nicely together and make it easier to manipulate and plot data.

There are 9 core tidyverse packages that you'll use the majority of the time you work with R in this class, and a couple dozen other packages that get *installed* when you install tidyverse, but aren't automatically *loaded*.

The core tidyverse packages are:

- [{dplyr}](https://dplyr.tidyverse.org/): Contains functions for manipulating data (like filtering, grouping, summarizing, mutating, etc.)
- [{tibble}](https://tibble.tidyverse.org/): Contains functions for working with data frames in a nicer way
- [{ggplot2}](https://ggplot2.tidyverse.org/): Contains functions for making pretty plots
- [{readr}](https://readr.tidyverse.org/): Contains functions for reading data into R in a nicer way
- [{tidyr}](https://tidyr.tidyverse.org/): Contains functions for reshaping data (like making wide data long and long data wide)
- [{stringr}](https://stringr.tidyverse.org/): Contains functions for working with strings, or text (like searching text, capitalizing/lowercasing text, replacing text, etc.)
- [{forcats}](https://forcats.tidyverse.org/): Contains functions for working with factors, or categories
- [{lubridate}](https://lubridate.tidyverse.org/): Contains functions for working with dates
- [{purrr}](https://purrr.tidyverse.org/): Contains functions for iterating across lots of values, like loops (you won't do much with it in this class)

The R command `library(tidyverse)` is a shortcut for loading all nine of these core tidyverse functions at the same time. You've probably noticed that when you run `library(tidyverse)`, a bunch of stuff appears:
...
↑ that's just showing that R loaded nine packages for you.

This means that you shouldn't really ever do something like this:
...
…because `library(tidyverse)` already loaded {dplyr} and {ggplot2}.

There are other non-core tidyverse packages too ([see here for the full list](https://tidyverse.org/packages/)). These are once you'll use in this class: 

- [{haven}](https://haven.tidyverse.org/): Contains functions for reading data from Stata, SPSS, and SAS files
- [{scales}](https://scales.r-lib.org/): Contains functions for formatting numbers nicely, like as percents and dollars and comma-separated numbers


### Why do you keep writing package names in `{}`s? Why do you include `()`s after function names?

Because that's kind of the norm in the R world. People tend to do one of three things with R package names:

- Curly brackets, like {ggplot2} and {dplyr}
- Bold text, like **ggplot2** and **dplyr**
- Normal text, like ggplot and dplyr

None of these approaches is wrong or bad. Some people like the bold text or normal text—the book [R for Data Science](https://r4ds.hadley.nz/) written by Hadley Wickham and coauthors (Hadley basically invented the tidyverse) uses both bold text and normal text.

I personally like the curly braces, in part because that's what R's help pages use. Look in the top left corner of any help page and it'll tell you (1) the name of the function and (2) the name of the package it lives in:

![](img/dplyr-curly-brackets.png){width="80%" fig-align="center"}

↑ {dplyr} is in curly brackets there.

With functions, pretty much everyone takes the same approach: write function names with a set of parentheses at the end to signal that it's a function. Most people will write function names in monospaced text too, which you can do with Markdown by wrapping the text in single backticks, like `` `ggplot()` ``.

Including the parentheses is useful because it distinguishes between functions like `mean()` and variables like `bill_dep` and `body_mass`.

So my personal style guide is to write like this:

> The {ggplot2} package lets you use `ggplot()` to create plots, while {dplyr} lets you use things like `group_by()` and `summarize()` to group and summarize data. You can use `filter()` to choose only rows where `body_mass` is greater than 3,000 grams.

Others might write like this:
...
and both approaches are fine too.
...
[See this for more details.](/news/2026-01-28_copy-paste-tweak.qmd)
...
In your extensions, many of you tried to make a histogram of a different column like flipper length, but kept the bin width at 200 from the body mass histogram, and it looked bad and wrong.
