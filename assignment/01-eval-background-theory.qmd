...
You need to complete the four sections listed below. Ideally you should type this in a Quarto document (.qmd) and render your document to HTML or Word or PDF, but you can also write in Word if you want (though your final project will need to be in Quarto, and this would give you practice).[^citations]

[^citations]: And if you want to be super brave, try using [Quarto's citation system](/resource/markdown.qmd#citations)!

**I've created a Quarto template you can use here:** [{{< fa file-archive >}} `background-theory.zip`](/projects/background-theory.zip). It's also [available on Posit.cloud](https://posit.cloud/spaces/737332/content/11811993).

You can draw your impact theory and logic model charts by hand or with something like [Diagrams.net](https://www.diagrams.net/), [Lucidchart](https://www.lucidchart.com/pages/home), or [Creately](https://creately.com/). Export the image as a PNG, place it in the same directory as your Quarto file, and include the image with Markdown.

The syntax for adding an image in Markdown is fairly simple. Importantly, **it is not R code**, so *don't* try putting it in an R chunk. Just type this (make sure the path to the image is relative to your .qmd file—if you put the PNG in a folder named `img`, for instance, use `img/whatever.png`):
...
### Part 1: Program background and purpose
