...
organ_ca_nj <- organ_donations_raw |>
  ...
  mutate(
    After = Quarter %in% c("Q32011", "Q42011", "Q12012"),
...
model_diff_diff <- lm(
  Rate ~ is_california + After + (is_california * After),
  data = organ_ca_nj
)

model_parameters(model_diff_diff, digits = 4)
```
...
model_twfe <- lm(Rate ~ Treated + State + Quarter, data = organ)
model_parameters(model_twfe)

model_twfe_nice <- feols(Rate ~ Treated | State + Quarter, data = organ)
model_parameters(model_twfe_nice)
```
