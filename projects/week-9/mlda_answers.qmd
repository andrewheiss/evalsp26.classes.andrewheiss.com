...
```{r}
model_al_ar <- lm(
  mrate ~ after_change + is_alabama + (after_change * is_alabama),
  data = al_ar
)
model_parameters(model_al_ar)
```
...
```{r}
model2 <- lm(
  mrate ~ legal + state_name + factor(year) + state_name:year,
  data = deaths_filtered
)
model_parameters(model2)

model2_fe <- feols(
  mrate ~ legal + i(state_name, year) | state_name + year,
  cluster = ~state_name,
  data = deaths_filtered,
  ssc = ssc(fixef.K = "full")
)
model_parameters(model2_fe)
```
