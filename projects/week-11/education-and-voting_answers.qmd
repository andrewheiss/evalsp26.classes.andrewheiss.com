...
model_parameters(model_logit, verbose = FALSE)
...
model_logit |> 
  avg_comparisons(variables = "college")
...
model_parameters(model_check_relevance, verbose = FALSE)
...
model_parameters(model_check_exclusion, verbose = FALSE)
...
```{r}
voting_with_prediction <- predictions(first_stage, voting) |> 
  rename(college_hat = estimate)
```
...
model_parameters(second_stage, verbose = FALSE)
```
...
model_parameters(model_all_at_once, verbose = FALSE)
...
model_feols <- feols(register ~ black + hispanic + female | college ~ distance,
                     data = voting)
model_parameters(model_feols, verbose = FALSE)
