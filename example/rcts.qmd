...
library(scales)  # Format numbers with functions like label_comma(), label_percent(), and label_dollar()
...
model_parameters(model_rct)
```

Based on your RCT, you conclude that the program causes an average increase of `r model_parameters(model_rct) |> filter(Parameter == "programProgram") |> pull(Coefficient) |> dollar()` in income.
