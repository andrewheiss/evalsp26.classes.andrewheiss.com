...
model_big <- lm(log_duration ~ highearn + after_1980 + highearn * after_1980 +
                ...
                data = injury_fixed)
model_parameters(model_big)

# Extract just the diff-in-diff estimate
diff_diff_controls <- model_parameters(model_big) |>
  filter(Parameter == "highearn:after_1980") |>
  pull(Coefficient)
```
...
modelsummary(list("Simple" = model_small, "Full" = model_big),
  ...
  style_tt(i = 7:8, background = "#F6D645")
```
