...
library(performance) # Extract model details to data frames
...
model_parameters(model_math_naive)
```
...
model_parameters(model_math_naive_z)
```
...
model_parameters(model_net_naive)

model_net_naive_z <- lm(malaria_risk_z ~ net, data = nets_scaled)
model_parameters(model_net_naive_z)
```
...
:::{.callout-warning}
**QUICK IMPORTANT CAVEAT**: Technically for mathy reasons, Cohen's *d* is [not exactly comparable](https://www.researchgate.net/post/How_can_I_calculate_an_effect_size_cohens_d_preferably_from_a_linear_random_effects_model_beta) to just looking at the number of standard deviations between the averages, but it's a good way of thinking about effect sizes. If you want to be official about it, one way to get Cohen's *d* from regression output is to divide the coefficient (which is like $\bar{x}_2 - \bar{x}_1$ in the formula) by the residual standard error of the model (which is like $s$ in the formula). You can get these from `model_parameters()` and `model_performance()`:
:::

```{r cohens-d-from-regression}
model_parameters(model_math_naive)
model_performance(model_math_naive)  # We want sigma here
3.89 / 9.43

# Or more programmatically without handwritten numbers
catholic_coef <- model_parameters(model_math_naive) |> filter(Parameter == "catholic_fCatholic") |> pull(Coefficient)
catholic_sigma <- model_performance(model_math_naive) |> pull(Sigma) 
catholic_coef / catholic_sigma

net_coef <- model_parameters(model_net_naive) |> filter(Parameter == "netTRUE") |> pull(Coefficient)
net_sigma <- model_performance(model_net_naive) |> pull(Sigma) 
net_coef / net_sigma
```
...
```{r naive-r2}
model_performance(model_math_naive)

model_performance(model_net_naive)
```
...
model_some_controls <- lm(malaria_risk ~ net + income + health,
                          data = nets)
model_performance(model_some_controls)
```
...
net_iptw <- predictions(model_net_treatment, nets) |> 
...
model_parameters(model_net_ate)

model_performance(model_net_ate)
```
...
We can convert this to a standardized mean difference (or Cohen's *d* effect size) by dividing the coefficient by the residual standard error (or `Sigma` in the results of `model_performance()`):
...
# Propensity scores and inverse probability of treatment weights (IPTW)
school_iptw <- predictions(model_school_treatment, school_data) |>
  rename(propensity = estimate) |> 
...
model_parameters(model_school_ate)

model_performance(model_school_ate)
```
...
To calculate different tipping points, we need to specify the confidence intervals of our causal effect. The easiest way to do that is to use `model_parameters()` and then filter those results so that we're just looking at our causal effect (the upper bound of the confidence interval for `netTRUE` in this case). We can then feed those results to `tip_coef()`, which handles the sensitivity analysis for us.
...
plot_stuff <- model_parameters(model_net_ate) |> 
  filter(Parameter == "netTRUE") |> 
  pull(CI_high) |>
...
plot_stuff <- model_parameters(model_school_ate) |> 
  filter(Parameter == "catholic") |> 
  pull(CI_low) |>
...
model_parameters(model_net_controls)
...
Printing out the results like this shows us a bunch of different estimates and robustness values, but I like the minimal reporting table that Carlos used in his video, so let's look at that instead.
...
model_parameters(model_school_controls)
