...
library(tinytable)  # Add fancier formatting to tables)
...
model_parameters(model_wrong)
```

According to this estimate, using a mosquito net is associated with a `r model_parameters(model_wrong) |> filter(Parameter == "netTRUE") |> pull(Coefficient) |> abs()` point decrease in malaria risk, on average. We can't legally talk about this as a causal effect though—there are confounding variables to deal with.
...
model_matched_wts <- lm(malaria_risk ~ net,
                        data = matched_data_for_real,
                        weights = weights)
model_parameters(model_matched_wts)
```

After weighting to account for under- and over-matching, we find a `r model_parameters(model_matched_wts) |> filter(Parameter == "netTRUE") |> pull(Coefficient)` point causal effect. That's much better than any of the other estimates we've tried so far! The reason it's accurate is because we've closed the confounding backdoors and isolated the arrow between net use and malaria risk.
...
You can make R exponentiate the coefficients automatically by including `exponentiate = TRUE` in `model_parameters()`:
...
logit_df_odds <- predictions(example_logit, logit_df, type = "link") |>
  rename(log_odds = estimate) |>
  mutate(odds_ratio = exp(log_odds))

logit_df_probs <- predictions(example_logit, logit_df, type = "response") |>
  rename(pred_prob = estimate)
...
# model_parameters(model_net, exponentiate = TRUE)
```
...
```{r make-p-scores}
# predictions() handles the plugging in of values. You need to feed it the name of the model and the name of the dataset you want to add the predictions to. 
net_probabilities <- predictions(model_net, nets) |>
...
model_parameters(model_ipw)
```

Cool! After using the inverse probability weights, we find a `r model_parameters(model_ipw) |> filter(Parameter == "netTRUE") |> pull(Coefficient)` point causal effect. That's a tiiiny bit off from the true value of 10, but not bad at all!
...
model_parameters(model_ipw_truncated)
```

Now the causal effect is `r model_parameters(model_ipw_truncated) |> filter(Parameter == "netTRUE") |> pull(Coefficient)`, which is slightly lower and probably less accurate since we don't really have any exceptional cases messing up our original IPW estimate.
...
modelsummary(list("Naive" = model_wrong,
  ...
  style_tt(i = 3:4, background = "#F6D645")
```
...
ipw_results <- model_ipw |> 
  model_parameters() |> 
  mutate(parameter = janitor::make_clean_names(Parameter)) |> 
  mutate(conf = glue::glue("[{round(CI_low, 2)}, {round(CI_high, 2)}]")) |> 
  split(~parameter)
...
So actually I lied. We're *not* all done yet. The IPW model that we ran gives us the correct point estimate (`r round(ipw_results$net_true$Coefficient, 2)`), but the standard error and confidence intervals are wrong. There's uncertainty inherent in the estimation of the propensity scores and inverse probability weights, and that uncertainty doesn't get passed on to the outcome model, thus resulting in underestimation of the standard error and overly narrow confidence intervals. 

We can actually see underestimation in the side-by-side table of results up above: the IPW model's standard error is `r round(ipw_results$net_true$SE, 2)`, resulting in a confidence interval of `r ipw_results$net_true$conf`, while the matching + weights model has a similar point estimate (`r round(matching_results$net_true$Coefficient, 2)`) but a bigger standard error (`r round(matching_results$net_true$SE, 2)`) and a wider confidence interval of `r matching_results$net_true$conf`.

The (arguably) most accurate way to include the uncertainty of the treatment model in the outcome model is to rely on brute force simulation to estimate the causal effect for lots and lots of different models and then combine them into a single value. With this data, we can't go out and collect new samples of mosquito net use—we only have one set of data to work with.
...
find_stats <- function(splits) {
  ...
  # The columns from model_parameters() have to be named specific things to 
  # work with int_t() later
  return(
    model_parameters(cookie_model) |>
...
fit_one_ipw <- function(split) {
  ...
  net_ipw <- predictions(model_net, current_data) |>
    mutate(ipw = (net_num / estimate) + ((1 - net_num) / (1 - estimate)))
...
The average net effect (or point estimate) of `r boot_stats$avg_net` is the same as what we found with the regular `lm()` model from before, but the standard error is now bigger (`r round(boot_stats$avg_se, 2)`) and more like the standard error we got with matching + weights (`r round(matching_results$net_true$SE, 2)`).
