...
model_adjusted <- lm(inspection_score ~ Weekend + NumberofLocations + Year, 
                     data = inspections)
model_parameters(model_adjusted)
```
...
freq_results_naive <- model_parameters(model_naive) |> 
  mutate(model = "Naive model") |> 
  filter(Parameter == "WeekendTRUE")

freq_results_full <- model_parameters(model_adjusted) |> 
...
ggplot(freq_results, aes(x = Coefficient, y = model, color = model)) +
...
The weekend point boost $\hat{\theta}$ is mostly clustered around 1–2, and 95% of those draws are between `r results$b_WeekendTRUE$CI_low_nice` and `r results$b_WeekendTRUE$CI_high_nice`. We're thus 95% sure that the actual weekend point boost is between `r results$b_WeekendTRUE$CI_low_nice` and `r results$b_WeekendTRUE$CI_high_nice` with a median of `r results$b_WeekendTRUE$Median_nice`. 

We can also look at this distribution a slightly different way by collapsing all those posterior draws into 100 possible values. Each of these dots is equally likely, and the true value of $\theta$ could be any of them, but again, most are clustered around `r results$b_WeekendTRUE$Median_nice`:
...
tribble(
  ...
  "How to do it in R", "`lm(...)`", "`library(brms)`<br>`brm(...)`",
...
We can do this manually with the posterior:
...
Or, conveniently, it's already included in the output from `model_parameters` as `pd`:
...
```{r}
model_parameters(model_bayes, null = 1)
```
...
weekend_draws |> 
  summarize(prop_outside_rope = sum(b_WeekendTRUE >= -0.5 & b_WeekendTRUE <= 0.5) / n())
```

We can also do this automatically with `model_parameters()`, though by default it only shows the proportion *in* the ROPE, unless you also add `complement = TRUE`:

```{r}
model_parameters(model_bayes, test = "rope", complement = TRUE)
```
